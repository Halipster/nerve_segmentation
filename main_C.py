'''
This script train a convolutional network on the data outputed from A1, A2, and
A3 in order to decide if a nerve is present or not.
'''
...
x_train = scale( (np.load('y_pred_A1.npy') + np.load('y_pred_A1.npy') + np.load('y_pred_A1.npy'))/3 )
...
for epoch in range(nb_epoch):
    ...
    if hist.history['loss'][0] < loss:
        model.save_weights('weights_C1.hdf5', overwrite=True)
        loss = hist.history['loss'][0]

model.load_weights('weights_C1.hdf5')
y_pred = model.predict(x_train, verbose=1)
np.save('y_pred_C1', y_pred)
np.save('y_true_C1', y_train)
