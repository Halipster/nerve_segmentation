'''
This script train a U-NET on all the data using scale_3 and cleaned data. 
'''
...
for epoch in range(nb_epoch):
    ...
    if hist.history['loss'][0] < loss:
        model.save_weights('weights_A3.hdf5', overwrite=True)
        loss = hist.history['loss'][0]

model.load_weights('weights_A3.hdf5')
x_train, y_train = generator.get_eval_set()
y_pred = model.predict(x_train, verbose=1)
np.save('y_pred_A3', y_pred)
